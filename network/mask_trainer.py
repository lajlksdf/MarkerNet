...
from network.helper import cal_iou
...
class MaskTrainer(BaseTrainer):
    ...
    def eval(self, item: MaskTrainItem):
        ...
        with torch.no_grad():
            images, out_aux = self.net_g(item.fake_data.to(self.device))
            figureUtil.mask_result(images.detach().cpu().squeeze(dim=2), item.mask_data.squeeze(dim=2),
                                   item.src_files, item.fake_files,
    ...
    def test(self, item: MaskTrainItem, net=None):
        ...
        with torch.no_grad():
            ...
            iou1, iou2 = cal_iou(item.mask_data, masks)
            self.iou1_ += iou1
            self.iou2_ += iou2
            self.count += 1
            figureUtil.mask_result(masks.detach().cpu().squeeze(dim=2), item.mask_data.squeeze(dim=2),
                                   item.src_files, item.fake_files,
