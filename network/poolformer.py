...
from network.modules.blocks import ResnetBlock
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, resblock=False,
            out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = ResnetBlock(in_features, hidden_features, 1)
        self.act = act_layer()
        self.fc2 = ResnetBlock(hidden_features, out_features, 1) if resblock else nn.Conv2d(hidden_features,
                                                                                            out_features, 1)
...
def basic_blocks(dim, index, layers,
        ...
        drop_rate=.0, drop_path_rate=0., resblock=False,
    ...
    for block_idx in range(layers[index]):
        ...
        blocks.append(PoolFormerBlock(
            dim, dim, pool_size=pool_size, mlp_ratio=mlp_ratio,
            act_layer=act_layer, norm_layer=norm_layer,
            drop=drop_rate, drop_path=block_dpr,
            use_layer_scale=use_layer_scale, resblock=resblock,
...
class PoolFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
            ...
            pool_size=3, in_chans=3, resblock=True,
        ...
        for i in range(len(layers)):
            stage = basic_blocks(embed_dims[i], i, layers,
                                 ...
                                 act_layer=act_layer, norm_layer=norm_layer,
                                 drop_rate=drop_rate, resblock=resblock,
...
@register_model
def poolformer_s6(fork_feat=False, in_chans=12, num_classes=1, **kwargs):
    """
    ...
    model = PoolFormer(
        layers, embed_dims=embed_dims, in_chans=in_chans, fork_feat=fork_feat,
        mlp_ratios=mlp_ratios, downsamples=downsamples, num_classes=num_classes,
        **kwargs)
...
@register_model
def poolformer_s12(fork_feat=False, in_chans=12, num_classes=1, resblock=True, **kwargs):
    """
    PoolFormer-S12 model, Params: 12M
    --layers: [x,x,x,x], numbers of layers for the four stages
    --embed_dims, --mlp_ratios:
    ...
    downsamples = [True, True, True, True]
    model = PoolFormer(
        layers, embed_dims=embed_dims, in_chans=in_chans, fork_feat=fork_feat, resblock=resblock,
        mlp_ratios=mlp_ratios, downsamples=downsamples, num_classes=num_classes,
        **kwargs)
    model.default_cfg = default_cfgs['poolformer_s']
    return model
...
@register_model
def poolformer_s24(fork_feat=False, in_chans=12, num_classes=1, resblock=True, **kwargs):
    """
